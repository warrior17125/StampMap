...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                ...
                activation="relu", num_element=50,normalize_before=False,
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        src = src.permute(1, 0, 2)  # [c, b_s, d_model]
        
        bs, num_frame = pos_embed.shape
        pos_embed = pos_embed.view(bs, num_frame, 1, 1).expand(-1, -1, self.num_element,
                                                               self.d_model)  # [b_s, h_f, ele_pad, d_model]
        pos_embed = pos_embed.flatten(1, 2).permute(1, 0, 2) * 0.1  # [h_f*ele_pad, b_s, d_model] : [c, b_s, d_model]
        query_embed = query_embed.permute(1, 0, 2)  # [track_num, b_s, d_model]

        tgt = torch.zeros_like(query_embed)
        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed)  # [c, b_s, d_model]
        hs = self.decoder(tgt, memory, memory_key_padding_mask=mask, pos=pos_embed,
                          query_pos=query_embed)  # [1, track_num, b_s, d_model]
        return hs.squeeze(0).transpose(0, 1), memory.permute(1, 0, 2)  # [b_s, track_num, d_model] [b_s, c, d_model]
